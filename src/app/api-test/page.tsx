          ...
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <p><span className="font-bold">Year:</span> {movieDetail.year}</p>
                <p><span className="font-bold">Quality:</span> {movieDetail.quality}</p>
                <p><span className="font-bold">Language:</span> {movieDetail.lang}</p>
                <p><span className="font-bold">Duration:</span> {movieDetail.time}</p>
              </div>
              ...
              {movieDetail.episodes && movieDetail.episodes.length > 0 && (
                <div className="mt-4">
                  ...
                  <div>
                    {movieDetail.episodes.map((server: any, index: number) => (
                      <div key={index} className="mb-2">
                        <p className="font-semibold">{server.server_name}</p>
                        <div className="flex flex-wrap gap-2">
                          {server.items.slice(0, 5).map((episode: any, episodeIndex: number) => (
                            <span key={episodeIndex} className="bg-gray-200 text-black px-2 py-1 rounded text-sm">
                              {episode.name}
                            </span>
                          ))}
